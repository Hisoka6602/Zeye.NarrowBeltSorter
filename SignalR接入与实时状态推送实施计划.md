...
4. 保持现有分层边界：Core 定义契约，Execution 负责采集编排，Worker Host（Generic Host）与 Web Host（Web 承载层）负责对外通信承载。
...
当前 `Zeye.NarrowBeltSorter.Host/Program.cs` 使用 `Host.CreateApplicationBuilder`（Worker 或 Generic Host）且未启用 Web 路由映射功能，SignalR 落地需先在以下两条路径二选一：
