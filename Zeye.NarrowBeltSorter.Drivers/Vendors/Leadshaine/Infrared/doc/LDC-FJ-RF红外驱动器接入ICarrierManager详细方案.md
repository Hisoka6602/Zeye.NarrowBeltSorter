...
1. 手册入库后的最低验收标准：  
   - `ConnectAsync` 连续调用 10 次无异常击穿，连接状态转换可回放。  
   - 握手、读状态、断连重连在日志中具备可追溯操作编号。  
   - 真实命令字与 CRC 算法均可映射到手册页码。  
2. 手册未入库前的临时验证范围：仅验证桥接链路连通性、收发超时、重试与异常日志，不验证具体命令语义。  
3. 手册缺失超过 3 个工作日时，升级到项目负责人 + 供应商接口人；在工单中记录阻塞原因、责任人、下一次跟进时间。
...
5. 手册未入库阶段的结构验证：仅验证帧边界（STX/ETX）、长度字段一致性、CRC 计算函数稳定性，不验证真实命令字。
...
5. 暂停后的处理流程：记录阻塞工单 -> 指派责任人（项目负责人）-> 向供应商确认 -> 结论回填文档后恢复联调。
...
1. 先补齐原始手册文件：`LDC-FJ-RF分拣专用红外驱动器使用说明书-20240618.pdf`，预期存放路径：`Zeye.NarrowBeltSorter.Drivers/Vendors/Leadshaine/doc/`。  
2. 按“握手/控制/状态/故障”分类录入真实命令，逐条附手册页码。  
3. 每条命令必须包含真实请求 Hex、真实应答 Hex、字段含义、CRC 计算方法。  
4. 文档与源码同步更新，禁止仅改文档不落库实现。
5. 若仓库内无手册文件：由项目负责人向供应商获取同版本手册，校验版本号后再入库；版本变更必须同步更新文档中的页码引用。
