...
- 解析成功后触发 `ChuteAssignedReceived` 事件；`ParcelHostedService` 订阅该事件并更新包裹目标格口与拓扑归属，驱动后续路径重算与分拣执行。
...
- 对齐“目标格口下发消息”的最小字段集与可选扩展字段，保证解析兼容与向后兼容。
...
- 上游是否要求固定消息外层 Envelope（除 `Type` 外是否需要版本号、站点号、签名字段）。
