...
MANUAL_RULES: set[int] = {40, 41, 42, 43, 44, 48, 50, 51, 52, 53, 54, 55, 56, 57, 58}
...
EXPECTED_RULE_TEXTS = {
    ...
    31: "强制：`appsettings.json` 与各模块基础配置文件（`appsettings.*.json`、`appsettings.devices.*.json`）的每个字段都必须有中文注释；环境覆盖文件（`appsettings.{Env}.*.json`）只需为覆盖的字段添加注释。",
    ...
    58: "强制：所有落盘日志文件单个大小不得超过 10MB；NLog 配置中所有 File target 必须设置 `archiveAboveSize=\"10485760\"` 并配合 `archiveFileName` 实现超限轮转，禁止无上限单文件写入。",
}
