...
├── 逐文件全量审查实施方案.md              # 逐文件全覆盖审查方案，覆盖影分身/性能/并发/规则合规并支持分批次多 PR 无遗漏检查
...
- `逐文件全量审查实施方案.md`：提供逐文件无遗漏审查方法、分批次执行顺序、记录模板与验收门禁、支撑“先审查后修复”的多 PR 推进模式。

## 本次更新内容

- 新增《逐文件全量审查实施方案.md》，用于指导 Copilot 按文件全覆盖执行质量与规则审查，可按多个 PR 拆分推进。
- 更新 README 文件树与逐文件职责说明，补充新方案文档条目，避免新增文件遗漏。

## 后续可完善点

- 将“逐文件审查记录模板”落地为每批次独立检查单，按批次沉淀问题清单与修复 PR 映射关系。
- 在 CI 中增加“审查批次覆盖率统计”，自动校验当批文件清单是否存在未勾选项。
