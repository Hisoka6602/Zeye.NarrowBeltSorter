...
├── 锁格调用与赋值链路分析.md                # 锁格能力在生产与测试中的调用入口、状态赋值与冲突校验链路分析
...
├── 上车匹配时序偏移详细分析与实时计划.md      # 论证上车匹配时序偏移理论并给出“基于实时速度”的实施计划
...
│   ├── Manager/Sorting
│   │   └── ILoadingMatchRealtimeSpeedProvider.cs # 上车匹配实时速度提供接口（补偿计算层与环轨管理器解耦）
...
│   │   ├── LoadingMatchCompensationState.cs # 上车匹配时序补偿状态枚举（Active/Fallback）
...
│   ├── Options/Sorting
│   │   └── SortingTaskTimingOptions.cs     # 分拣任务时序配置（包裹成熟延迟、成熟起始来源、格口开关门间隔、链路告警阈值、上车触发滞后窗口、步距/补偿开关/滞回阈值/平滑窗口/速度合法范围）
...
│   ├── Utilities/SensorTimeHelper.cs       # 传感器触发时间解析工具（OccurredAtMs → 本地 DateTime，合法性校验+统一入口）
│   ├── Utilities/DelayRatioIntervalStats.cs # 延迟占比区间分桶统计工具（0~50%/50~80%/80~95%/95%+，无锁线程安全）
│   └── Utilities/SortingValueFormatter.cs  # 分拣链路数值格式化工具（速度/延迟"最多两位小数，整数不带小数位"规范）
...
│       ├── SortingTaskCarrierLoadingService.cs # 分拣上车编排服务（成熟队列消费、上车绑定、Carrier-Parcel映射、五段链路时刻记录、P50/P95/P99统计、时序补偿：EffectiveDelayMs/DelayRatio/CarrierPeriodMs观测、滞回+平滑+delta偏移）
...
│   └── appsettings.chutes.devices.json     # 全环境统一格口设备参数分片（IP/端口/格口映射/红外参数）
└── Zeye.NarrowBeltSorter.Core.Tests                   # 单元与集成测试层：覆盖 Core/Drivers/Execution 核心行为
    ...
    ├── SensorTimeHelperTests.cs            # SensorTimeHelper 单元测试（合法值、零/负/超上限、DateTimeKind 校验）
    ├── DelayRatioIntervalStatsTests.cs     # DelayRatioIntervalStats 单元测试（四档分桶边界值、负值、极大值、并发安全）
    ├── SortingValueFormatterTests.cs       # SortingValueFormatter 单元测试（整数不带小数位、两位小数规范、N/A 行为）
...
- `锁格调用与赋值链路分析.md`：梳理锁格能力在生产链路与测试链路的调用入口、状态赋值路径与冲突校验路径。
...
- `上车匹配时序偏移详细分析与实时计划.md`：对“触发后时间推进导致上车错位”进行公式化分析，并给出必须接入实时速度的阶段化实施计划。
...
- 新增《上车匹配时序偏移详细分析与实时计划.md》：完成“步距/速度/延迟占比”模型分析，明确补偿必须依赖实时速度。
- 上车日志补充实时速度：`SortingTaskCarrierLoadingService` 的“上车位装车成功/超阈值告警”日志新增 `LoopTrackRealtimeSpeedMmps` 字段。
- 落格日志补充实时速度：`SortingTaskDropOrchestrationService` 的“落格成功/超阈值告警”日志新增 `LoopTrackRealtimeSpeedMmps` 字段。

## 后续可完善点

- 在上车匹配补偿落地时，按实时速度动态计算步距周期并输出占比分桶统计，形成错位率量化闭环。
- 增补“实时速度缺失/异常抖动”回归测试，验证日志字段完整性与补偿降级路径可追溯性。
