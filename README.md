...
- 统一轨道配置来源：移除 `appsettings.devices.looptrack.json`，仅保留 `appsettings.looptrack.json` 承载轨道参数，消除同键覆盖冲突。
- 调整格口设备分片命名：将 `appsettings.devices.chutes.json` 重命名为 `appsettings.chutes.devices.json`，避免“设备总入口”误导。
- 修复 `SignalTowerHostedService` 启动预警蜂鸣并发竞态：旧代际会话不再覆盖最新状态，运行/检修态强制关闭蜂鸣器，避免首次启动长鸣不止。
- 调整轨道日志参数策略：为避免热路径配置读取开销，`LoopTrackManagerHostedService` 日志相关参数改为启动快照，不支持运行期热更新。
- 收敛实时速度日志噪音：急停、停止、故障状态下抑制实时速度日志输出。
- 补齐系统状态变更落盘日志：`LocalSystemStateManager` 在每次状态切换前统一记录结构化日志。
- 调整格口“关闭全部”语义：停机阶段改为清空强排集合而非锁格，保持“默认不锁格”。
- 新增《锁格调用与赋值链路分析.md》：归档锁格调用入口、赋值路径与冲突校验关系。

## 后续可完善点

- 为 `SignalTowerHostedService` 增补“启动预警取消与状态快速切换”并发回归测试，覆盖首启按键与连续切态场景。
- 如需恢复 `LoopTrackManagerHostedService` 日志参数热更新，建议通过非热路径缓存刷新机制实现（避免在状态循环热路径直接读取配置）。
