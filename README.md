...
- `SignalR接入与实时状态推送实施计划.md`：基于现有实时发布契约与编排服务，规划 SignalR 无鉴权接入、连接首帧全量下发、五类主题实时增量推送与客户端连接方式。
...
- 文档内覆盖 `looptrack/emc/system/carrier/orchestration` 五类主题的实时数据范围与验收清单，并补充 JavaScript/.NET 客户端连接示例。
